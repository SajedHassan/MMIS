...
def validate(args, net, val_loader, opt, writer=None, times_step = 0):
    ...
    with torch.no_grad():
        for val_step, (patch, masks, sid) in enumerate(tqdm(val_loader)):
            ...
            if args.stage == 1:
                outputs = net.val_step(patch, sample_num = args.val_num)
            elif args.stage == 2:
                outputs = net.test_step(patch, sample_num = args.val_num)
...
def evaluate(net, test_loader, opt, args, result_path):
    ...
    with torch.no_grad():
        for test_step, (patch, masks, sid) in enumerate(tqdm(test_loader)):
            ...
            if args.stage == 1:
                outputs = net.val_step(patch, sample_num = args.test_num)
            elif args.stage == 2:
                outputs = net.test_step(patch, sample_num = args.test_num)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default='../models/DPersona1_LIDC_20240318-231123/', help="save path")
    ...
    for fold_idx in range(opt.KFOLD):
        ...
        ckpt = torch.load(os.path.join(args.save_path,'{}{}_{}_{}_best.pth'.format(args.model_name, args.stage, opt.DATASET, fold_idx)))
