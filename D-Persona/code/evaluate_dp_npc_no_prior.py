...
from dataloader.dataset_no_prior import BaseDataSets, ZoomGenerator
...
def validate(args, net, val_loader, opt, writer=None, times_step = 0):
    ...
    with torch.no_grad():
        for val_step, sample in enumerate(tqdm(val_loader)):
            ...
            generated_masks = sample['generated_label'].cuda()

            preds = []
            for idx in range(patch.shape[2]):
                if args.stage == 1:
                    output_slice = net.val_step(patch[:,:,idx], generated_masks[:,:,idx], sample_num = args.val_num)
                elif args.stage == 2:
...
def evaluate(net, test_loader, opt, args, result_path):
    ...
    with torch.no_grad():
        for test_step, sample in enumerate(tqdm(test_loader)):
            ...
            for idx in range(patch.shape[2]):
                if args.stage == 1:
                    output_slice = net.val_step(patch[:,:,idx], generated_masks[:,:,idx], sample_num = args.test_num)
                elif args.stage == 2:
