...
class DPersona(nn.Module):
    def __init__(self, input_channels=1, num_classes=1, num_filters=[16,32,64,128,256],
        ...
        self.proj_heads_1 = Projection(self.num_experts, self.latent_dim).to(device)
        self.proj_heads_2 = Projection(self.num_experts, self.latent_dim).to(device)
        self.proj_heads_3 = Projection(self.num_experts, self.latent_dim).to(device)
        self.proj_heads_4 = Projection(self.num_experts, self.latent_dim).to(device)
    ...
    def train_step(self, args, images, masks, annotator, loss_fct, stage = 1):
        if stage == 1:
            ...
            annotator = torch.tensor(annotator)
            single_path_y_pred = y_preds[:, annotator, :, :]
            # single_path_y_pred_np = single_path_y_pred.cpu().detach().numpy()
            # single_path_y_pred_np_0 = single_path_y_pred_np[0]
            # single_path_y_pred_np_1 = single_path_y_pred_np[1]
            # single_path_y_pred_np_2 = single_path_y_pred_np[2]
            # single_path_y_pred_np_3 = single_path_y_pred_np[3]
            # single_path_y_pred_np_4 = single_path_y_pred_np[4]
            single_path_mask = masks[:, annotator, :, :]
            # single_path_mask_np = single_path_mask.cpu().detach().numpy()
            # single_path_mask_np_0 = single_path_mask_np[0]
            # single_path_mask_np_1 = single_path_mask_np[1]
            # single_path_mask_np_2 = single_path_mask_np[2]
            # single_path_mask_np_3 = single_path_mask_np[3]
            # single_path_mask_np_4 = single_path_mask_np[4]
            loss = loss_fct(single_path_y_pred, single_path_mask)
        return loss, single_path_y_pred
    ...
    def test_step(self, images, sample_num = 100):
        ...
        for idx in range(self.num_experts):
            z_expert, z_expert_org = getattr(self, 'proj_heads_' + str(idx + 1))(self.unet_features, z_set, idx)
            sample_expert = self.z_mapping(z_expert)
            np_sample_expert = sample_expert.cpu().detach().numpy()[0][0]
            np_sample_expert_0 = self.z_mapping(torch.unsqueeze(z_set[:,0,:], axis=1)).cpu().detach().numpy()[0][0]
            np_sample_expert_10 = self.z_mapping(torch.unsqueeze(z_set[:,10,:], axis=1)).cpu().detach().numpy()[0][0]
            np_sample_expert_20 = self.z_mapping(torch.unsqueeze(z_set[:,20,:], axis=1)).cpu().detach().numpy()[0][0]
            np_sample_expert_30 = self.z_mapping(torch.unsqueeze(z_set[:,30,:], axis=1)).cpu().detach().numpy()[0][0]
            np_sample_expert_40 = self.z_mapping(torch.unsqueeze(z_set[:,40,:], axis=1)).cpu().detach().numpy()[0][0]
