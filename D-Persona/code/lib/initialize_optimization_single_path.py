...
def init_optimization(model, args):
    ...
    if args.model_name == 'pionono':
        ...
        opt_params = [
            {'params': model.proj_heads_1.parameters()},
            {'params': model.proj_heads_2.parameters()},
            {'params': model.proj_heads_3.parameters()},
            {'params': model.proj_heads_4.parameters()}
        ]
        optimizer1 = torch.optim.Adam([opt_params[0]], lr=learning_rate)
        optimizer2 = torch.optim.Adam([opt_params[1]], lr=learning_rate)
        optimizer3 = torch.optim.Adam([opt_params[2]], lr=learning_rate)
        optimizer4 = torch.optim.Adam([opt_params[3]], lr=learning_rate)
    ...
    if not (args.model_name == 'DPersona' and args.stage == 2):
        optimizer = torch.optim.Adam(opt_params, lr=learning_rate)
        return loss_fct, optimizer, None, None, None

    return loss_fct, optimizer1, optimizer2, optimizer3, optimizer4