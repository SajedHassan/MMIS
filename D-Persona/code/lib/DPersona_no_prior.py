...
class DPersona(nn.Module):
    ...
    def kl_divergence(self, analytic=True, calculate_posterior=False, z_posterior=None):
        ...
        if analytic:
            ...
            prior_dist = Independent(Normal(loc=torch.zeros_like(self.posterior_latent_space.base_dist.loc), scale=torch.ones_like(self.posterior_latent_space.base_dist.scale)), 1)
            kl_div = kl.kl_divergence(self.posterior_latent_space, prior_dist)
        else:
    ...
    def elbo(self, args, segm, criterion, analytic_kl=True):
        ...
        self.ged_loss = generalized_energy_distance_with_torsh(segm, self.re_prior)
        return -(self.reconstruction_loss + self.kl + args.beta * self.bound_loss), self.ged_loss

    def combined_loss(self, args, labels, loss_fct):
        elbo, ged = self.elbo(args, labels, criterion=loss_fct)
        self.reg_loss = (l2_regularisation(self.posterior) + l2_regularisation(self.fcomb.layers)) * self.reg_factor
    ...
    def val_step(self, images, masks, sample_num = 50):
        ...
        y_pred = torch.cat(self.posterior_sampling(sample_num=sample_num, training=False), dim=1)
        return y_pred
