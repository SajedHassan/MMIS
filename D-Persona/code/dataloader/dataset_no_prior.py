...
class BaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train" or self.split == "gen":
            ...
            generated_h5f = h5py.File(self._base_dir + "/validation_generated/{}".format(case), "r")
        elif self.split == "test":
            h5f = h5py.File(self._base_dir + "/testing/{}".format(case), "r")
            generated_h5f = h5py.File(self._base_dir + "/testing_generated/{}".format(case), "r")
        ...
        if self.split == "train" or self.split == "gen":
            ...
            label[2] = h5f["label_a3"][:]
            label[3] = h5f["label_a4"][:]
            generated_label = np.zeros((4, image.shape[1], image.shape[2], image.shape[3]))
            generated_label[0] = generated_h5f["gen_label_a1"][:]
            generated_label[1] = generated_h5f["gen_label_a2"][:]
            generated_label[2] = generated_h5f["gen_label_a3"][:]
            generated_label[3] = generated_h5f["gen_label_a4"][:]
            sample = {"image": image, "label": label, "generated_label": generated_label}
...
class ZoomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, generated_label = sample["image"], sample["label"], sample["generated_label"]

        if self.split == 'gen':
            image = np.expand_dims(image, axis=1)
            label = np.expand_dims(label, axis=1)
            generated_label = np.expand_dims(generated_label, axis=1)
        ...
        generated_label = zoom(generated_label, (1, 1, self.output_size[0] / x, self.output_size[1] / y), order=0)

        image = torch.from_numpy(image.astype(np.float32))
        label = torch.from_numpy(label.astype(np.uint8))
        generated_label = torch.from_numpy(generated_label.astype(np.uint8))
        sample = {"image": image, "label": label, "generated_label": generated_label}
        return sample
